...
router.get('/suite-performance', async (req, res) => {
  try {
    ...
    // Fetch all suites
    const suites = await AppDataSource.getRepository('Suite').find();

    // 1. Bookings Count query
    const bookingsCountQuery = AppDataSource.getRepository('Booking')
      .createQueryBuilder('b')
      .select('b.suiteId', 'suiteId')
      .addSelect('COUNT(b.id)', 'count')
      .where('b.status != :cancelled', { cancelled: 'cancelled' });
    if (start) bookingsCountQuery.andWhere('b.createdAt >= :start', { start });
    if (end) bookingsCountQuery.andWhere('b.createdAt <= :end', { end });
    const bookingsCountRows = await bookingsCountQuery.groupBy('b.suiteId').getRawMany();

    // 2. Revenue query
    const revenueQuery = AppDataSource.getRepository('Payment')
      .createQueryBuilder('p')
      .innerJoin('p.booking', 'b')
      .select('b.suiteId', 'suiteId')
      .addSelect('SUM(p.amount)::numeric::float8', 'total')
      .where("p.status = 'success'");
    if (start) revenueQuery.andWhere('p.createdAt >= :start', { start });
    if (end) revenueQuery.andWhere('p.createdAt <= :end', { end });
    ...
    const ratingQuery = AppDataSource.getRepository('Review')
    ...
    const globalAvgRow = await AppDataSource.getRepository('Review')
      .createQueryBuilder('r')
    ...
    const bookingTableName = AppDataSource.getRepository('Booking').metadata.tableName;
    const repeatQuery = AppDataSource.getRepository('Booking')
      .createQueryBuilder('b')
      .select('b.suiteId', 'suiteId')
      .addSelect('COUNT(b.id)', 'count')
      .where('b.status != :cancelled', { cancelled: 'cancelled' })
    ...
    if (start) repeatQuery.andWhere('b.createdAt >= :start', { start });
    if (end) repeatQuery.andWhere('b.createdAt <= :end', { end });
    ...
    function getSlotsPerDay(suite: any): number {
